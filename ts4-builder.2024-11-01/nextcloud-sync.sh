...
# nextcloud backup path
NEXTCLOUD_BACKUP_PATH="${NEXTCLOUD_PATH}/${NEXTCLOUD_PRODUCT_PATH}/_downloading"
...
handleSyncQuery () {
   ...
   do
      ...
      local NEXTCLOUD_PROJ_PATH=${NEXTCLOUD_SYNC_PATH}/${PRODUCT_PROJECT}
      ...
      if [[ ${ARG_REPOS} == *${PRODUCT_REPO}* ]]; then
         if [[ ${HANDLING_REPOS} != *${PRODUCT_REPO}* ]]; then
            ...
            eval "${CURL_CMD} --request MOVE -H 'Destination: ${NEXTCLOUD_BACKUP_PATH}/${PRODUCT_PROJECT}' '${NEXTCLOUD_PROJ_PATH}'"
...
handleSyncQueries () {
   ...
   if [ ! -z "${HANDLING_REPOS}" ]; then
      local CLEARING_REPOS=${HANDLING_REPOS}
      echo "############################# clear trashbin. HANDLING_REPOS=${HANDLING_REPOS}, CLEARING_REPOS=${CLEARING_REPOS} ###############################"
      for (( index = 0; index < ${#REPO_PRODUCTS_ARRAY[@]}; index = index + 4 ))
      do
         local PRODUCT_REPO=${REPO_PRODUCTS_ARRAY[index]}
         if [[ ${CLEARING_REPOS} != *${PRODUCT_REPO}* ]]; then
            echo "CLEARING_REPOS=${CLEARING_REPOS}, PRODUCT_REPO=${PRODUCT_REPO}, is not exist, continue..."
            continue
         fi
         CLEARING_REPOS=${CLEARING_REPOS#${PRODUCT_REPO} }
         echo "CLEARING_REPOS=\'${CLEARING_REPOS}\', PRODUCT_REPO=\'${PRODUCT_REPO}\'"
         ...
         # if [ ${CANCEL_RESULT} -eq 0 ] ; then
            echo "nextcloud-sync::handleSyncQueries: ${PRODUCT_REPO} is canceled. ${NEXTCLOUD_PROJ_PATH} will be restored."
            ${CURL_CMD} --request DELETE ${NEXTCLOUD_PROJ_PATH}

            echo "restore ${PRODUCT_PROJECT} directory."
            eval "${CURL_CMD} --request MOVE -H 'Destination: ${NEXTCLOUD_PROJ_PATH}' '${NEXTCLOUD_BACKUP_PATH}/${PRODUCT_PROJECT}'"

            continue
         fi
         echo "nextcloud-sync::handleSyncQueries: clear backup ${PRODUCT_PROJECT} directory."
         ${CURL_CMD} --request DELETE ${NEXTCLOUD_BACKUP_PATH}/${PRODUCT_PROJECT}
      done
