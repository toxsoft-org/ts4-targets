...
buildAll () {
  ...
  buildTarget ${SKF_USERS_REPO} ${TS4_SKIDE_REPO} ${GIT_MAIN_BRANCH} ${BUILD_MODE} ${OUTPUT_TO_GLOBAL}
  case $? in
  ...
  buildTarget ${SKF_REPORTS_REPO} ${SKF_JOURNALS_REPO} ${GIT_MAIN_BRANCH} ${BUILD_MODE} ${OUTPUT_TO_GLOBAL}
  case $? in
  ...
  case $? in
     0 ) BUILDED_REPOS="${BUILDED_REPOS} ${TS4_DEVS_REPO}";BUILD_MODE="${FORCE}";; 
     1 ) ERRORED_REPOS="${ERRORED_REPOS} ${TS4_DEVS_REPO}";;
     2 ) CANCELED_REPOS="${CANCELED_REPOS} ${TS4_DEVS_REPO}";;
     * ) 
  esac
